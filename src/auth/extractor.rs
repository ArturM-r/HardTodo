...
impl AuthUser{
    ...
    pub fn from_jwt(state: &AppState, header: &HeaderValue) -> Result<Self, AppError> {
        let data = header.to_str().map_err(|_| { debug!("Header is wrong"), AppError::UNAUTHORIZED })?;
...
impl FromRequestParts<AppState> for AuthUser{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &AppState) -> Result<Self, Self::Rejection> {
        let auth_header = parts.headers.get(AUTHORIZATION).ok_or(AppError::UNAUTHORIZED)?;
